...
float computeReprojectionError(float* pError, const CvPoint3D32f* p3D, const CvPoint2D32f* p2D, int nPoints, const float* pRot, const float* pTrans, float f) {
	/**
...
void calcHomography(float* pResult, const CvPoint2D32f* pQuad) {
	...
	pResult[2] = ((c[0].x + c[1].x - c[2].x - c[3].x) * pResult[6] + (-c[0].x + c[1].x + c[2].x - c[3].x) * pResult[7]) / -4;
	pResult[5] = ((c[0].y + c[1].y - c[2].y - c[3].y) * pResult[6] + (-c[0].y + c[1].y + c[2].y - c[3].y) * pResult[7]) / -4;
